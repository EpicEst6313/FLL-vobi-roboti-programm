# 1. moodul

  ## Ehitus ja eesmärgid
 
See moodul korjab platsilt ühe krilli ja ühe proovidest, vabastab hai koopast ja lükkab üles
korallipungad ning tuleb tagasi kodualasse

Algrõhk sellel moodulil on 2 bari või u 30 psi (ehk u 200 000 Pa, vist)

Punkte:
- -10 p (korrallrahu segmentide ära toomine ) + 20 p (korallipungad) + 20 p (Sukelduja ära toomine) + 20 p (hai koobast vabastamine) + 5 p (veeproov) = 55 p

Kui kaua võtab sõit aega: 17.01 (55 p)

paigutamine platsil:

<img src="roboti%20pildid/20250211_144552.jpg" alt="Alt Text" width="500">       <img src="roboti%20pildid/20250211_144557.jpg" alt="Alt Text" width="500">
<img src="roboti%20pildid/20250211_144609.jpg" alt="Alt Text" width="500">

pildid moodulist:

- jassu pane pildid

  ## Head küljed

- Kuna seda on katsetatud väga palju kordi siis see on ka tänu sellele väga täpseks muutunud ja pneumaatika töötab alati

  ## Probleemid

- see moodul libiseb vastu maad seda oli/on väga keeruline mudeliks teha

# 2. moodul

  ## Ehitus ja eesmärgid
**Uus (17.02.25) 2. moodul**

Võtab kraakeni haardest kirstu, samal ajal tõstab masti, seejärel paneb robot Sukelduja korallrahu toele.

Algasend võistlusplatsil:

<img src="roboti%20pildid/20250217_172351.jpg" alt="Alt Text" width="500">        <img src="roboti%20pildid/20250217_172401.jpg" alt="Alt Text" width="500">

pildid moodulist:

- jassu pane pildid

  ## Head küljed

- see töötab peaaegu 100%

  ## Probleemid

- sõidab liiga kiiresti vahel masti otsa ja lükkab ümber

  ## Miks tegime ümber?
- Endine 2. moodul oli ebastabiilne, kaalult raske, lagunes kergelt.
- Endine 2. moodul oli suure missioonihulga pärast ebatäpsem kui uus
- Endisel moodulil polnud võimalik kasutada värvituvastust
- Kergem platsil paika panna
- Kergem programmeerida
- Küljes värvituvastuse toimimiseks spetsiaalne klots

Pildid vanast moodulist:

- jassu pane pildid

  ## Miks ei sobinud vana moodul?
- Tegi liiga palju ülesandeid, mis muutus liiga keeruliseks, seetõttu ka ebatäpseks. Seda moodulit oli liiga keeruline üles seada, samuti raiskas ta liiga palju aega ning vasakpoolne tõstmisasi kasutas metallkuule, mis oli kole ja ebamugav.


  # 3. moodul
  ## Ehitus ja eesmärgid

  viib ära hai vaskust kodualast hai elupaika ning võtab nelijala ja läheb paremasse kodu alasse

  ## Head küljed

  - kompaktne
  - töötab väga hästi
  - on väga lihtne programmi kirjutada

  ## Puudused

  -võiks rohkem punkte teenida

  ## Miks sellise varindi tegime?

  - eelmine versioon oli kohutav
  - eelmine versioon ei teinud asju korralikult ära
  - oli kohutav progeda

## miks ei sobinud vana moodul?

Oleks ära toonud kolmhargi (kahjuks praktikas ei saanud me seda töötma) ja viis hai haide elupaika. Pärast kolmharki välja tõstmist korjas krillid platsilt ja ühe korallrahu segmenti ning pidi nelijala ära tegema 

Miks ei kasuta?
Ehitus kvaliteet on kehv, moodul ei töötanud. Pealegi oli seda väga raske progeda. Käpp (või värk), mis krille püüdis oli raske ning kukkus alalõpmata maha, takistades moodulil teiste missioonide tegemist.


  # 4.moodul
  ## Ehitus ja eesmärgid


  ## Head küljed


  ## Puudused


  ## Miks sellise variandi tegime?


  ## Muudatused


   # 5. moodul?(Heiki palun seleta)
   ## Ehitus ja eesmärgid
 

  ## Boonused


  ## Probleemid


  ## Miks sellise variandi tegime?


  ## Muudatused

